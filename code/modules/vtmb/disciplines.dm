...
/datum/daimonion/proc/baali_get_clan_weakness(target, caster)
	...
	if(iskindred(vampire))
		...
		if(clan.get_discipline("Thaumaturgy") && !(vampire.clane?.name == "Tremere"))
	...
	switch(level_casting)
		if(1)
			...
			if(mypower < 3)
				...
				caster.emote("stare")
				if(mypower == -1)
					to_chat(target, "<span class='warning'>You feel someone's presence attempt to read your soul!</span>")
					caster.do_jitter_animation(10)
				return
			if(!ishuman(target))
				to_chat(caster, "<span class='warning'>[target] doesn't have enough mind to get affected by this discipline!</span>")
		...
		if(2)
			...
			if(mypower < 3)
				...
				caster.emote("stare")
				if(mypower == -1)
					var/mob/living/carbon/human/frenzied_caster = caster
					frenzied_caster.enter_frenzymod()
					addtimer(CALLBACK(frenzied_caster, TYPE_PROC_REF(/mob/living/carbon, exit_frenzymod)), 5 SECONDS)
					caster.do_jitter_animation(10)
				return
			if(!ishuman(target))
				to_chat(caster, "<span class='warning'>[target] doesn't have enough mind to get affected by this discipline!</span>")
		...
		if(5)
			...
			if(mypower < 3)
				...
				if(mypower == -1)
					...
					to_chat(target, "<span class='warning'><b>You feel like someone tried to place a curse on you!</span</b>")
				return
			if(!ishuman(target))
				to_chat(caster, "<span class='warning'>[target] doesn't have enough mind to get affected by this discipline!</span>")
				return
